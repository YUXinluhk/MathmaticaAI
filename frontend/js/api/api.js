...
window.app.api = {
    // The BASE_URL property has been removed to ensure all functions use the single source of truth from state.js

    testBackendConnection: async function() {
        try {
            // FIX: Use the consistent and correct URL from the state object.
            const response = await fetch(`${window.app.state.BACKEND_URL}/api/test-connection`, {
                method: 'POST',
    ...
    generateLatexDocument: async function(requestBody) {
