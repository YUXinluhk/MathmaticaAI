...
function initializeUI() {
    updateModelOptions();
    updateConnectionStatus(); // Add this line to sync UI on load
...
async function startRigorousVerification() {
    ...
    while (systemState.verificationState.currentIteration < systemState.verificationState.maxIterations &&
        ...
        try {
            ...
            let improvedSolution = await callAI('self_improve', { problem: problem, prompt: improvementPrompt, solution: currentSolution, rigor: rigor });
            
            // Logic to handle AI 'laziness'
            if (!improvedSolution.toLowerCase().includes('solution') && !improvedSolution.toLowerCase().includes('proof')) {
                improvedSolution = currentSolution; // If AI doesn't return the full solution, use the original one
            }
...
async function testAIConnection() {
    ...
    try {
        // Use a valid task type 'generate_python' and provide all necessary data placeholders.
        const response = await callAI('generate_python', { 
            prompt: "Say 'Hello, World!'",
            code: "", // Provide empty code to match prompt template
            solution: "This is a test." // Add the missing 'solution' placeholder
        });
        if (response) {
        ...
        updateConnectionStatus(); // Always update UI to reflect final state
    }
}
